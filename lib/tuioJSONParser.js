...
function tuioJSONParser(options) {
	...
	function parsePenMessage(message) {
		...
		if (message.words) {
			parsePenHWMessage(message);
		} else {
			parsePenPointMessage(message);
		}
		
		return true;
	}
	
	function parsePenHWMessage(message) {
		var event	= new HandwritingEvent('handwriting',{
			words: message.words;
		});
		that.eventDispatcher.dispatch(event, null);
	}
	...
	function HandwritingEvent(eventName,initialData){
		...
		event.initUCustomEvent(eventName, true, true, 1);
		properties(event, props);
		inject(event, initialData);
		return event;
	}
