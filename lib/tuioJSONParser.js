...
function tuioJSONParser(options) {
	...
	function parseTouchMessage(message) {
		...
		switch (message.state) {
			...
			case 'move':
				if (!Touches[message.id]) {
					if (options.fixStartEventLack) {
						...
						if (options.throwErrors) throw "No preluding touchstart event found for touchmove event (Id.:"+message.id+")";
						success = false;
					}
				} else {
					dispatchTouchmove(message.id,x,y);
			...
			case 'end':
				if (!Touches[message.id]) {
					if (options.throwErrors) throw "No preluding touchstart found for touchremove event (Id.:"+message.id+")";
					success = false;
				} else {
	...
	function parseGestureMessage(message) {
		...
		if (message.x) {
			inject(message, calculatePosition(message.x, message.y));
		}
	...
	function parseScaleGestureMessage(message) {
		switch(message.state) {
			...
			case 'gesturechange':
			case 'grestureend':
				var gestureEvent	= new GestureEvent(message.type+message.state,{
					scale:		message.scale,
					rotation:	0
				});
				that.eventDispatcher.dispatch(gestureEvent);
				break;
		}
		return true;
	}
	...
	function parseRotateGestureMessage(message) {
		switch(message.state) {
			...
			case 'gesturechange':
			case 'grestureend':
				var target			= (message.x) ? document.elementFromPoint(message.x, message.y) : null;
				var gestureEvent	= new GestureEvent(message.tpye+message.state,{
					target:		target,
					scale:		1,
					rotation:	message.rotation
				});
				inject(gestureEvent, calculateBrowserPositions(message.x, message.y));
				that.eventDispatcher.dispatch(gestureEvent);
				break;
		}
		return true;
	...
	function parseCustomGestureMessage(message) {
		switch(message.state) {
			case 'start':
				console.log('new gesture',message);
