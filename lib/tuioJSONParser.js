...
function tuioJSONParser(options) {
	...
	function parsePenMessage(message) {
		...
		if (message.words) return parsePenHandwritingMessage(message);
		else return parsePenPointMessage(message);
	}
	...
	function parsePenHandwritingMessage(message) {
		var event	= new HandwritingEvent(message.state,{
			words: message.words
		});
	 ...
	 * Pen point events are interpreted as mouseenter / mousemove / mouseleave event.
	 * 
	 * @param	message		The message object
	 * @return	success
	 */
	function parsePenPointMessage(message) {
		...
		switch (message.state) {
			case 'start':
				eventName				= (options.interpretPenAsMouse) ? 'mouseenter' : 'penstart';
				PenTargets[message.id]	= getTarget(x,y);
				break;
			case 'move':
				eventName				= (options.interpretPenAsMouse) ? 'mousemove' : 'penmove';
				break;
			case 'end':
				eventName				= (options.interpretPenAsMouse) ? 'mouseleave' : 'penend';
				break;
		}

		// (1) create PenEvent object
		var data		= calculateBrowserPositions(x,y);
	...
	function MouseEvent(eventName, initialData) {
		...
		event.shiftKey =		initialData.shiftKey		|| false;
		event.metaKey =			initialData.metaKey 		|| false;
		event.button =			initialData.button			|| 0;
		event.relatedTarget =	initialData.relatedTarget	|| null;
