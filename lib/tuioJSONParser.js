...
function tuioJSONParser(options) {
	...
	this.parse = function(message) {
		...
		if (message['gesture-type']!='drag' && message['type']!='touch'){	
			console.log("incoming message to parse:",message);
		}
		var success = false;
		switch(message.type) {
	...
	function parseRotateGestureMessage(message) { 
		console.log('rotate state'+message.state);
		...
		switch(message.state) {
			...
			case 'end':
				var target;
				for(var i=0;i<message.touches;i++) {
