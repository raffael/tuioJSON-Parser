...
function tuioJSONParser(options) {
	...
	options = extend({
		...
		coordinateCalibration: {
			topLeft: { x:0, y:0},			// in px
			bottomRight: {x:undefined, y:undefined}			// in px
		},
		pen: {
	...
	function calculatePosition(x,y) {
		if (options.useBrowserRelativeCoordinates) {
			...
			return {
				x: parseInt(screen.width*x - options.coordinateCalibration.topLeft.x),
				y: parseInt(screen.height*y - options.coordinateCalibration.topLeft.y)
			};
		} else {
			return {
