...
/**
 * TO DO
 * - all kind of pen events do not contain identifier information yet.
 */
...
function tuioJSONParser(options) {
	...
	this.parse = function(message) {
		...
		switch(message.type) {
			...
			case 'touch':
				success = parseTouchMessage(message);
				break;
			
			case 'gesture':
				success = parseGestureMessage(message);
				break;
			...
			case 'shape':
				success = parseShapeMessage(message);
				break;
			
			case 'handwriting':
				success = parseHandwritingMessage(message);
				break;
		}
		return success;
	}
	...
	/**
	 * PenPoints
	 * is a container object to store PenPoint objects under their identifier.
	 */
	var PointCollection		= {
		'touch':	{},
		'pen':		{}
	...
	function parseShapeMessage(message) {
		var event	= ShapeEvent(message.type+message.shapeType,message);
	...
	function getTargetTouches(type, element, excludeTouchByIdentifier) {
		var result	= document.createTouchList();		
		for(var i in PointCollection[type]) {
			if (PointCollection[type][i].target == element && (!excludeTouchByIdentifier || i!=excludeTouchByIdentifier)) result.push(PointCollection[type][i]);
		}
		return result;
	}
