...
function tuioJSONParser(options) {
	...
	this.setOptions = function(newOptions) {
		console.log("Resetting tuioJSON Parser options.");
	...
	function parseTouchMessage(message) {
		...
		function dispatchTouchstart(identifier,x,y) {
			...
			var touchEvent	= TouchEvent(options.touch.touchstartName,{
				touches:		getTouches(),
		...
		function dispatchTouchmove(identifier,x,y) {
			...
			var touchEvent	= TouchEvent(options.touch.touchmoveName,{
				touches:		getTouches(),
		...
		function dispatchTouchend(identifier,x,y) {
			...
			// (2) create TouchEvent object
			var touchEvent	= TouchEvent(options.touch.touchendName,{
				identifier:		identifier,
	...
	function parseDragGestureMessage(message) {
		...
		var dragEvent	 = GestureEvent(message.gestureType+message.state, {
	...
	function parsePenPointMessage(message) {
		...
		var event		= PenEvent(eventName, calculateBrowserPositions(x,y));
	...
	function parsePenShapeMessage(message) {
		var event	= ShapeEvent(message.type+message.penType,message);
	...
	function parsePenHandwritingMessage(message) {
		...
		var event	= HandwritingEvent(message.penType+message.state,data);
		return self.eventDispatcher.dispatch(event, document);
	}
	...
	function getTouches(excludeTouchByIdentifier) {
		if (!excludeTouchByIdentifier) {
			if (!currentTouchList) {
				currentTouchList	= TouchList();
				rebuildTouchList();
			}
			return currentTouchList;
		} else {
			var result	= TouchList();		
